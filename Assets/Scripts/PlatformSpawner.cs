...
public class PlatformSpawner : MonoBehaviour {
    ...
    public float yMin = -3.5f; // 배치할 위치의 최소 y값(랜덤 사용)
    public float yMax = 1.5f; // 배치할 위치의 최대 y값(랜덤 사용)
    private float xPos = 20f; // 배치할 위치의 x 값(고정값, 화면 밖에서 생성)
    ...
    void Update() {
        ...
        if(Time.time >= lastSpawnTime + timeBetSpawn)
        {
            ...
            // 다음 배치까지의 시간 간격을 timeBetSpawnMin, timeBetSpawnMax 사이에서 설정
